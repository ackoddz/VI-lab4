...
graph = UndirectedGraph({
    "B": {"A": dist('B','A'), "I": dist('B', 'I'), "C": dist('B','C')},
    "I": {"H": dist('I','H'), "J": dist('I','J')},
    "C": {"F": dist('C','F'), "G": dist('C','G'), "D": dist('C','D')},
    "F": {"G": dist('F','G'), "J": dist('F','J')},
    "K": {"G": dist('K','G'), "L": dist('K','L')},
    "D": {"E": dist('D','E'), "L": dist('D','L')},
    "M": {"L": dist('M','L')}
})
...
dolzina1 = astar_search(graph_problem).path_cost
graph_problem = GraphProblem(Stanica1,Kraj,graph)
dolzina2 = astar_search(graph_problem).path_cost
final1 = dolzina1 + dolzina2

graph_problem = GraphProblem(Pocetok,Stanica2,graph)
dolzina1 = astar_search(graph_problem).path_cost
graph_problem = GraphProblem(Stanica2,Kraj,graph)
dolzina2 = astar_search(graph_problem).path_cost
final2 = dolzina1 + dolzina2

if(final1 < final2 or final1 == final2):
    graph_problem = GraphProblem(Pocetok,Stanica1,graph)
    rez1 = astar_search(graph_problem).solve()
    graph_problem = GraphProblem(Stanica1,Kraj,graph)
    rez2 = astar_search(graph_problem).solve()
    print(rez1 + rez2[1:])
else:
    graph_problem = GraphProblem(Pocetok,Stanica2,graph)
    rez1 = astar_search(graph_problem).solve()
